...
const CONFIG = {
  ...
  detailedCheckItems: [
    ...
    // Check for key files
    {
      path: '.roo/rules/01-standard-toml-md-format.md',
      name: 'TOML-MD format rules',
      type: 'file',
    },
    {
      path: '.roo/rules-roo-commander/01-operational-principles.md',
      name: 'Commander principles',
      type: 'file',
    },
  ],
...
class Logger {
  ...
  warning(msg) {
    return this._write('WARNING', msg)
  }
  error(msg) {
    return this._write('ERROR', msg)
  }
  success(msg) {
    return this._write('SUCCESS', msg)
  }
  ...
  async close() {
    if (this.stream) {
      try {
        await this.info(
          `=== Roo Commander Update Script Completed: ${new Date().toISOString()} ===\n`,
        )
        await this.stream.close()
        this.stream = null
      } catch (e) {
    ...
    res.body.on('data', (chunk) => {
      ...
      if (totalSize > 0) {
        ...
        if (percent % 10 === 0 && percent > lastLoggedPercent) {
          if (VERBOSE)
            logger.info(`Download progress: ${percent}% (${downloadedSize}/${totalSize})`)
...
async function getCurrentVersion() {
  ...
  try {
    ...
    if (version) {
      ...
      await logger.warning(
        `Invalid version format in ${path.basename(CONFIG.versionFile)}: ${versionData.version}`,
      )
    }
  } catch (e) {
    if (e.code !== 'ENOENT') {
      await logger.warning(
        `Could not read or parse ${path.basename(CONFIG.versionFile)}: ${e.message}`,
      )
  ...
  try {
    ...
    if (versionMatch && versionMatch[1]) {
      ...
      if (inferredVersion) {
        await logger.info(
          `Inferred version from ${path.basename(CONFIG.requiredFiles[0])}: ${inferredVersion}`,
        )
    ...
    if (e.code !== 'ENOENT') {
      await logger.warning(
        `Could not read or parse ${path.basename(CONFIG.requiredFiles[0])} for inference: ${e.message}`,
      )
    }
  }
...
async function pruneBackups() {
  ...
  try {
    ...
    if (backupDirs.length > CONFIG.maxBackups) {
      ...
      await logger.info(
        `Found ${backupDirs.length} backups, removing ${backupsToRemove.length} oldest...`,
      )
...
async function validateBaseInstallation() {
  ...
  try {
    ...
    const content = await fs.readFile(roomodesPath, 'utf8')
    if (!content.includes('"slug": "roo-commander"')) {
      // Simple check
      await logger.error(
        `Invalid ${path.basename(CONFIG.requiredFiles[0])}: Missing basic "roo-commander" slug entry.`,
      )
      return false
    }
  } catch (e) {
    await logger.error(
      `Cannot read or validate ${path.basename(CONFIG.requiredFiles[0])}: ${e.message}`,
    )
    return false
  }
...
async function performDetailedInstallationCheck() {
  ...
  try {
    ...
    if (!Array.isArray(roomodesObj.customModes)) {
      logger.error(
        `Invalid ${path.basename(CONFIG.requiredFiles[0])} structure: customModes is not an array.`,
      )
      checksPassed = false
    } else {
    ...
    logger.error(
      `Failed to read or parse ${path.basename(CONFIG.requiredFiles[0])} for detailed check: ${error.message}`,
    )
    checksPassed = false
  }
  ...
  if (!checksPassed) {
    ...
    if (missingModes.length > 0)
      logger.warning(`- Missing required modes: ${missingModes.join(', ')}`)
...
async function runUpdater() {
  ...
  if (CHECK_INSTALLATION) {
    ...
    if (detailedCheckOk) {
      ...
      await logger.info(
        'Consider running the update without --check-installation, possibly with --force.',
      )
      return false // Indicate failure
    }
  }
  ...
  if (!FORCE_UPDATE) {
    const confirmed = await confirmWithUser(
      `Do you want to ${action} Roo Commander ${latestVersion}?`,
    )
    if (!confirmed) {
  ...
  if (isInstalled) {
    try {
      ...
      const proceedAnyway = await confirmWithUser(
        'Backup failed. Proceed with update WITHOUT backup?',
      )
      if (!proceedAnyway) {
  ...
  if (!baseValidationOk) {
    ...
    if (backupPath) {
      ...
      try {
        ...
        await logger.error(
          `CRITICAL: Restore failed after update validation error: ${restoreError.message}`,
        )
    ...
    throw new Error(
      'Post-update validation failed. Check logs. Restore attempted if backup was made.',
    )
  }
...
async function main() {
  ...
  try {
    ...
    if (!success && !CHECK_INSTALLATION) {
      // Set error code if update/install failed or was cancelled
      exitCode = 1
    } else if (!success && CHECK_INSTALLATION) {
      // Set error code if check failed
      exitCode = 1
    }
  } catch (error) {
    await logger.error(`-----------------------------------------`)
