...
1.  **Logging Requirement:**
    ...
    - Follow the standard procedure defined in the workspace rule **`.kilocode/rules/08-logging-procedure-simplified.md`**.

2.  **Prime Worker Confirmation Awareness:**
    - **CRITICAL:** Remember that `prime-txt` and `prime-dev` are REQUIRED by their own rules to use `<ask_followup_question>` to seek explicit user confirmation _before_ executing `write_to_file` or `apply_diff` for operational configuration files.
